...
object GetFollowStatIncrement {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val eventCardData = eventCardImpress
      ...
      .withColumn("oweTo", isBefore($"impressTime", $"followTime"))
      .groupBy("creatorId")
      .agg(count("impressTime").as("eventCardImpressCount"), sum("oweTo").as("eventCardFollowCount"))

    // 合并和保存最终结果
    val finalResult = addFollowData.join(eventCardData, Seq("creatorId"), "outer")
